...
v_unordered_set<Literal*> MLMatcher2::Impl::getMatchedAlts()
{
...
v_unordered_map<unsigned, TermList> MLMatcher2::Impl::getBindings()
{
  MatchingData* md = &s_matchingData;
  ASS(!md->resolvedLit);  // Untested if using this together with resolvedLit works correctly
...
MLMatcher2::MLMatcher2(Literal** baseLits, unsigned baseLen, Clause* instance, LiteralList** alts, Literal* resolvedLit, bool multiset)
  : m_impl{std::make_unique<MLMatcher2::Impl>()}
{
...
v_unordered_set<Literal*> MLMatcher2::getMatchedAlts()
{
  ASS(m_impl);
  return m_impl->getMatchedAlts();
}

v_unordered_map<unsigned, TermList> MLMatcher2::getBindings()
{
  ASS(m_impl);
